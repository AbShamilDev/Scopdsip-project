...
const LabTableRow = (props) => {
  ...
  const deleteLab = async () => {
    const deleteWrapClasses = document.getElementById(
      `deleteAnimWrapper_${props.number}`
    ).classList;
  ...
  return (
    <tr>
      <td>{props.number}</td>
      <td>{props.title}</td>
      <td>{`${beginDate.getDate() < 10 ? `0${beginDate.getDate()}` : beginDate.getDate()}.${
        beginDate.getMonth() + 1 < 10 ? `0${beginDate.getMonth() + 1}` : beginDate.getMonth() + 1
      }.${beginDate.getFullYear()}`}</td>
      <td>{`${deadLine.getDate() < 10 ? `0${deadLine.getDate()}` : deadLine.getDate()}.${
        deadLine.getMonth() + 1 < 10 ? `0${deadLine.getMonth() + 1}` : deadLine.getMonth() + 1
      }.${deadLine.getFullYear()}`}</td>
      <td>
      ...
      <ActionSelector
        editLab={props.editLab}
        labInfo={restFn(props)}
        deleteLab={deleteLab}
        link={props.link}
      />
    </tr>
